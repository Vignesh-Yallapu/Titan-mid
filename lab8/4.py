...
class ModifiedAgent:
    """Agent using epsilon-greedy policy with exponentially weighted moving average."""

    def _init_(self, nActions, eps, alpha):
        self.nActions = nActions  # Number of actions
        self.eps = eps  # Probability of exploration
        self.alpha = alpha  # Step size for the moving average
        self.Q = np.zeros(nActions, dtype=float)  # Action-value estimates

    def update_Q(self, action, reward):
        # Update Q-value for action using the exponentially weighted moving average
...
def experiment(n_actions, N_episodes, eps, alpha, stddev):
    ...
    agent = ModifiedAgent(n_actions, eps, alpha)  # Initialize modified agent
    actions, rewards = [], []
...
N_steps = 10000  # Number of steps per experiment (increased to 10,000 as requested)
N_experiments = 10  # Reduced number of experiments for visualization
eps = 0.1  # Exploration probability
alpha = 0.1  # Step size for non-stationary rewards
stddev = 0.01  # Standard deviation for the random walk
save_fig = True
output_dir = os.path.join(os.getcwd(), "output")

# Run non-stationary bandit experiments
print("Running non-stationary bandits with nActions = {}, eps = {}, alpha = {}, stddev = {}".format(n_actions, eps, alpha, stddev))
R = np.zeros((N_steps,))  # Reward history
A = np.zeros((N_steps, n_actions))  # Action history

for i in range(N_experiments):
    actions, rewards = experiment(n_actions, N_steps, eps, alpha, stddev)
